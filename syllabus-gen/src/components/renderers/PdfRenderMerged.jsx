...
import { getDeptName } from "../../config/resetFormData.js";
import jsPDF from "jspdf";
import { Download } from "lucide-react";

export function PdfRenderMerged({department,courses}){
    ...
    async function renderCoursePDFPage(doc, courseData,isFirstPage) {
  const data = courseData || {};
  
  // ❌ REMOVE the new jsPDF() HERE
  ...
  function addHeader() {
      ...
      try {
        doc.addImage(header_img, "PNG", x, y, headerWidth, headerHeight);
      }catch (e) {
    ...
    return bottom + 18;
      ...
      if (Array.isArray(data.modules) && data.modules.length > 0) {
        for (let i = 0; i < data.modules.length; i++) {
          ...
          const title = `Module ${i + 1}: ${mod.title || ""}`;
  ...
  if (data.copoMapping && Array.isArray(data.copoMapping.rows)) {
    ...
    doc.setFont("Tinos-Regular","normal");
    // HEADERS (POs + PSOs)
    const poHeaders = ["CO", ...data.copoMapping.headers, "PSO1", "PSO2"];
    ...
    const copoRows = data.copoMapping.rows.map((row) => {
      return [
        ...
        row.pso1 || "",       // new PSO1 column
        row.pso2 || ""        // new PSO2 column
      ];
    });
    ...
    const totalRow = [
      "Total",
      ...copoRows[0].slice(1).map((_, colIndex) =>
        copoRows.reduce((sum, row) => sum + (Number(row[colIndex + 1]) || 0), 0)
    )
  ];
  ...
  //FOOTER
      ...
      // Save
      // doc.save((data.fileName || "syllabus") + ".pdf");
      // doc.addPage()
    ...
    async function generateAllCoursesPDF(courses) {
        
        const doc = new jsPDF({ unit: "pt", format: "a4" });
        for (let i = 0; i < courses.length; i++) {
            if (i !== 0) doc.addPage();

            //Getting individual PDF
            await renderCoursePDFPage(doc, courses[i],i===0);
        }
        
        doc.save(`${department}_All_Subjects.pdf`);
}

    //Calling GenerateAllPdfs Functino
    function downloadAllPdfs() {
        generateAllCoursesPDF(courses);
    }
    return(
        <button
